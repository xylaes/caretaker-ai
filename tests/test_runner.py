...
def test_command_runner_failure():
    ...
    runner = CommandRunner(
        "python -c \"import sys; sys.stderr.write('error_log'); sys.exit(42)\""
    )
    success, output, code = runner.run()
...
def test_command_runner_stderr_whitespace_fallback():
    # Run a command that writes only a space to stderr, but writes "stdout_data" to stdout
    runner = CommandRunner(
        "python -c \"import sys; sys.stderr.write(' '); sys.stdout.write('stdout_data')\""
    )
    success, output, code = runner.run()
    assert success is True
    assert output == "stdout_data"
    assert code == 0
